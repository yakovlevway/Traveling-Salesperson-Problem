...
class humanplayer:
    def evaluate(self,board):
 # Poluchenie posicii i posicii drugix igrokov
        me=tuple(board[0:2])
        others=[tuple(board[x:x+2]) for x in range(2,len(board)-1,2)]
 # Postroenie doski
        for i in range(4):
 ...
 # Pokazati xodi, dlya spravki
        print ('Ваш последний ход %d' % board[len(board)-1])
        print (' 0')
        print ('2 3')
        print (' 1')
        print ('Введите ход: ',)
 # Vernut vvedennoe polzovatelem chislo
        move=int(input( ))
        return move
...
flist=[addw,mulw,ifw,gtw,subw]
winner=evolve(5,100,tournament,maxgen=400)
